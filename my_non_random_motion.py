...
class my_NonRandomMotion(RandomTransform, IntensityTransform, FourierTransform):
    ...
    def apply_transform(self, subject: Subject) -> Subject:
        ...
        transform = my_NewMotion(**self.add_include_exclude(arguments))
        transformed = transform(subject)
        return transformed #data.subject.Subject (1, 320, 260, 48)
...
class my_NewMotion(IntensityTransform, FourierTransform):
    ...
    def apply_transform(self, subject: Subject) -> Subject:
        ...
        for image_name, image in self.get_images_dict(subject).items():
            ...
            result_arrays = []
            for channel in image.data:      #Loop over all image data for a particular subject
    ...
    def get_rigid_transforms(
        ...
        # Convert degrees_params and translation_params from lists 
        # into arrays here!
        center_ijk = np.array(image.GetSize()) / 2
        center_lps = image.TransformContinuousIndexToPhysicalPoint(center_ijk)
        # center_lps = image.get_center(lps=True)  #//HO 02.05.2023
        # center_lps = (85, 125, 125)  #Remove

        identity = np.eye(4)
        matrices = [identity]
        for degrees, translation in zip(degrees_params, translation_params):
            translation_arr = np.array(translation, dtype=float)  #//HO 20.06.2022
            ...
            motion.SetTranslation(translation)

            # HO 02.05.2023 end
            
            motion.SetCenter(center_lps)
            motion_matrix = self.transform_to_matrix(motion) #Euler3DTransformation matrix
            matrices.append(motion_matrix)
        transforms = [self.matrix_to_transform(m) for m in matrices]
        return transforms #Returns transformation matrices
    ...
    def sort_spectra(spectra: np.ndarray, times: np.ndarray):
        ...
        # HO 10.03.2023 start
        """Remove to not add non-physical artifacts to data"""
    ...
    def add_artifact(
        ...
        arrays = [sitk.GetArrayViewFromImage(im) for im in images] # ITK to NumPy
        arrays = [array.transpose() for array in arrays]
        spectra = [self.fourier_transform(array) for array in arrays]
        # HO 20.06.2022 start
        times_arr = np.array(times).flatten()
        # HO 10.03.2023 start
        # self.sort_spectra(spectra, times_arr)
        # HO 10.03.2023 end
        # HO 20.06.2022 end
        result_spectrum = np.empty_like(spectra[0])
        last_index = result_spectrum.shape[2] # The number has to match the dimension that the k-space filling is performed /HO 25.07.2023
        indices = (last_index * times_arr).astype(int).tolist()
        indices.append(last_index)
        ini = 0
        for spectrum, fin in zip(spectra, indices):
            # result_spectrum[..., ini:fin] = spectrum[..., ini:fin] # To get ghosting in the AP direction for OpenBHB coronal slices
            result_spectrum[:, ini:fin, :] = spectrum[:, ini:fin, :] # To get ghosting in the AP direction 26.10.2022 (Skyra Fit / MR-ART data / ADNI data)
            # result_spectrum[ini:fin, :, :] = spectrum[ini:fin, :, :] # To get ghosting in the AP direction for NAKO data
            ini = fin
